...
def getAltitude(pressure, airpressure): # 기압 - 높이 계산 Dtriple
  try:
    ...
    pass
def handleSyncData(mqtt_data, extAddress):
    print("handleSyncData")
    ...
    if dev is not None:
        ...
        try :
            ...
            print("****** error ********")
            print(e)
def handleGatewayState():
    print("handleGatewayState")
@mqtt.on_connect()
  ...
  if message.topic == '/efwb/post/sync':
    with thread_lock:
        if mqtt_thread is None:
          ...
          mqtt_thread = socketio.start_background_task(handleSyncData( mqtt_data,extAddress))
          mqtt_thread = None
  elif message.topic == '/efwb/post/connectcheck' :
      with thread_lock:
        if gw_thread is None:
          gw_thread = socketio.start_background_task(handleGatewayState(json.loads(message.payload)))
          gw_thread = None
