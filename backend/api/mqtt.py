...
def handle_gps_data(mqtt_data, extAddress):
    ...
    try:
        ...
        app_logger.error(f"Unexpected error proc5essing eHG4 GPS data: {str(e)}", exc_info=True)
  ...
  if dev is not None:
    ...
    try:
      ...
      if sensorDev is not None:
        ...
        app_logger.info(f"No existing walkRunCount found for band.")
    ...
    app_logger.info(f"An unregistered band: {extAddress}.")
